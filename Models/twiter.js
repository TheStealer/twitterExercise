...
const modelProducts = {
  aperturaDeArchivo: function () {
    //apertura de archivo
    const cadenaJsonA = fs.readFileSync(
  ...
  estructurarObjetoPOST: function (req) {
    ...
    const map = {
      ...
      mm:
        fecha.getMonth() + 1 <= 9
          ? "0" + (fecha.getMonth() + 1)
          : fecha.getMonth() + 1, //podriamos arreglar que agregue un cero en el mes
    ...
    const productoTmp = {
      ...
      discount: parseInt(
        req.body.price - (req.body.discountRate / 100) * req.body.price
      ), //precio a pagar menos el descuento
      ...
      registrationDate: "dd/mm/yy".replace(
        /dd|mm|yy|yyy/gi,
        (matched) => map[matched]
      ),
      checkInTime:
